...
Startup passes the selected options directly to `Simulation` and explicitly retains round 3, matching the previous constructor-plus-reset path and its round-seeded bot names. Default headless construction still starts at round 2. Keep both RNG state and subsequent round selection identical when changing initialization; the stress setup uses the same configuration as the existing stress fixture.
...
`npm run validate` writes accelerated simulation results to `artifacts/simulation-results.json`; those are not browser FPS measurements. The notebook at `/sloppy-tanks/benchmark.html` displays saved results.
