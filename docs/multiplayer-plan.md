...
- **M1b/M6 acceptance:** the original single-browser latency experiment chose the starting policy and has since been removed. The real client reconstructs projectiles that launch and hit between updates, so its two-browser check (with dev-only `?latency`, `?jitter` and `?stall` delay) and timeline tests replace the lab's incomplete combat sampling. The chosen starting policy is 20 Hz input, remote interpolation behind an adaptive playout buffer (one batch plus measured arrival lateness, independent of RTT) and bounded local and remote extrapolation, with a latency warning above 120 ms. This is a playtest default, not a claim that prediction is unnecessary at every RTT. Human control-feel judgment and several rounds on different networks remain required before production rollout.
...
| `src/net/interpolation.ts`          | Client: snapshot buffer, server-clock estimate, per-entity interpolation (`render-timeline.ts`)                                                          |
