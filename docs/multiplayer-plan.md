...
Status: revised proposal, 2026-09-23. Nothing here is implemented yet. Milestones are acceptance gates; the larger ones are split into separately reviewable pull requests below.
...
| Latency handling   | Remote entities interpolated ~100 ms behind; local aim immediate; local hull smoothing chosen by a single-player latency test (M1b) | No movement prediction initially. If that test fails, bring prediction into v1 before building the client.                                                                                       |
...
Reported from a 2026-09-23 cloud-container experiment (Intel Xeon 2.8 GHz, Node 22). These figures were not rerun during this plan revision and do not establish player-device or Cloudflare production performance. M1 must produce reproducible evidence using Node 24 or newer for local tooling, the pinned game dependencies, and the deployed Workers runtime.
...
- **Aggregate results across builds:** the same seed produced identical scores and body counts in Node (`@dimforge/rapier3d-compat`) and workerd (`@dimforge/rapier3d`) on all three maps. This is not a full-state determinism test; other browser engines were not tested.
- **Memory:** after forced garbage collection, a 3-minute match used ~1–3 MB of JavaScript heap and ~2 MB of Rapier memory. This excludes transient allocation peaks and does not establish total runtime memory or room isolation.
- **Traffic:** a quantized binary snapshot was estimated at 0.4–0.5 KB on average (about 1 KB at most) at 20 Hz, plus 12–17 game events per second. Colyseus's delta encoding measured 5.4–8.0 KB/s per player including events (one 20 s run per map). Neither figure measures this plan's JSON protocol. Measure serialized snapshots, events, input acknowledgements, and full-state bursts separately.

Keep scripts reproducible and record the commit, versions, seeds, sample counts, all maps, and outliers. Store run outputs under ignored `artifacts/performance/multiplayer/`; link the evidence in the implementation PR. Do not present historical estimates as a passed hosting or bandwidth gate. Once M1 has recorded current evidence, replace this section with a link to it; historical measurements belong in PR descriptions or local artifacts, not in this document.
...
- **Placement:** a Durable Object is created near its first request, so a room lives near whoever created it. Friends in other regions pay that distance in RTT. Record per-player RTT in M6; a `locationHint` can override placement if the default is poor.
...
| `src/net/mirror.ts`        | Client: applies validated full state and deltas to plain entity data, then supplies render state                                                             |
...
- Each client's snapshots report the highest input `seq` consumed for its seat. That supports input-to-visible delay measurements and later reconciliation. Input more than 500 ms behind the current server tick by `observedTick` is rejected, as are impossible future ticks. Test these limits in the M1b and M5 latency cases.
...
- `start { roomEpoch, roundId, seed, options, seats }` announces a round. A `control` message supplies `tankId`, `life` and the current `controlEpoch` on assignment, handoff and respawn. Input stays disabled until the matching full state or respawn state has arrived.
- `full { roomEpoch, roundId, seq, tick, elapsed, match, seats, entities, eventCursor }` on start, late join, resume, reconnect and resync. Include every current tank, cover (including destroyed covers and collapse rubble), pickup, shot, mine and fragment, using server-issued ids. Include settled transforms as well as moving ones. Fragment metadata includes shape, dimensions, color, wreck part/team, creation time and cleanup state; cover metadata includes hp, alive, timber hits and geometry/appearance changes. Full state must be sufficient without replaying earlier destruction events or relying on identical simulation initialization. If measured full-state size warrants it, untouched authored cover may be referenced by layout index instead of repeating its geometry, since a matching `contentVersion` guarantees the same layout.
- `snap { roomEpoch, roundId, seq, tick, elapsed, match, updates, removed, events }`, 20/s, one stream per round shared by all playing clients. Serialize the shared body once per snapshot. Each client's message wraps it with that seat's `ack { controlEpoch, inputSeq }`, added without re-serializing the body. `updates` contains explicitly typed entity creates/changes; `removed` contains entity type/id removals. Omission means unchanged, never deleted. Tank updates include life, pose, velocity, heading, aim, health/shield, recoil, rank, timers, ammo and kills/deaths. Include all fields required by rendering and HUD calculations.
- `result { roomEpoch, roundId, tick, scores, winner, players }`, `pong { t, tick }`, and explicit `room-reset` / `error` messages. A lost in-memory match returns players to the lobby with a reason, rather than pretending to resume it.
...
Order: M1 hosting experiment and M1b local latency spike → M2 simulation/view boundaries → M3 complete protocol → M4 deployed host → M5 full client flow → M6 friends playtest. M1b needs no server and no simulation changes, so it can run alongside M1. The two-browser version of its latency cases runs in M5 against the real network code.
...
The open question is whether your own tank feels acceptable without prediction when its motion arrives one round trip late, in 20 Hz samples. Remote interpolation 100 ms behind at 20 Hz is a well-established technique and is checked with two browsers in M5. Answer the own-tank question in the existing single-player game, before any refactor or server work:

- Add a dev-only URL option to the local game controller, off by default. It delays commands by half the simulated round trip before `sim.step`. It then samples the player's authoritative pose at 20 Hz and delivers the samples after the other half. The Simulation, its fixed step and its RNG order are unchanged; the delay and sampling live in the controller, plus a display-only pose override for the player's hull and camera in presentation.
- Draw the hull and camera from those samples under each candidate policy: the newest sample, continuous render-only smoothing, and bounded velocity extrapolation with correction. Do not extrapolate indefinitely through walls or across death/respawn. Keep turret aim immediate, drawn from the displayed hull toward the pointer; the delayed command carries the aim point, as the protocol does. Optionally draw other tanks from the same samples 100 ms behind.
- Test 0, 50, 100 and 150 ms of added round trip, a case with up to 30 ms variable delay that preserves order, and a short stall. Compare 20 and 30 Hz input sampling. Watch the hull and camera while starting, stopping, turning and colliding, and while shooting at moving targets. Record the input-to-visible delay each case produces.
...
| Friends far from the room's Durable Object get high RTT                   | Record per-player RTT in M6; use a `locationHint` if default placement is poor                                                                                    |
