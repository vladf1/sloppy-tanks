...
function duel(range = 18) {
  ...
  const human = s.human, bot = s.tanks.find((t) => t.team !== human.team)!;
  ...
  bot.aim = 0; bot.brain.decision = 0; bot.brain.ultraAggressive = false;
  s.world.step(); s.start();
  return { s, bot, human };
}

test("all roles appear, assignment is stable, and only every tenth bot is a hunter", () => {
  const rare = Array.from({ length: 100 }, (_, i) => botAssignment(i, i % 2, i))
    .flatMap((b, i) => b.ultraAggressive ? [i] : []);
  assert.deepEqual(rare, [9,19,29,39,49,59,69,79,89,99]);
  const s = new Simulation(123);
  const roster = s.tanks.filter((t) => !t.human).map((t) => [t.brain.personality, t.brain.ultraAggressive]);
  assert.equal(roster.filter(([, rare]) => rare).length, 1);
  assert.equal(new Set(roster.map(([role]) => role)).size, BOT_PERSONALITIES.length);
  s.reset();
  assert.deepEqual(s.tanks.filter((t) => !t.human).map((t) => [t.brain.personality, t.brain.ultraAggressive]), roster);
  s.dispose();
});
...
test("hunters seek through cover but cannot shoot through it; ordinary bots require sight", () => {
  ...
  bot.brain.personality = "guard";
  botCommand(s, bot, STEP); assert.equal(bot.brain.target, 0);
  bot.brain.ultraAggressive = true; bot.brain.decision = 0;
  const command = botCommand(s, bot, STEP);
  assert.equal(bot.brain.target, human.id); assert.equal(command.fire, false);
...
test("snipers actually hold their firing lane and minelayers deliberately deploy near enemies", () => {
  ...
  s.human.body.setTranslation({ x: 0, y: 0.65, z: 12 }, true); s.world.step();
...
test("a full track buffer cannot replace still-visible marks and can reuse fully faded ones", () => {
  ...
  for (let i = 4000; i < 4010; i++) {
    human.body.setTranslation({ x: i, y: 0.65, z: 0 }, true); trails.update(s, 1);
  }
