...
test("hull damage earns shared XP; only the finisher receives the kill bonus, without overkill XP", () => {
  ...
  assert.equal(ally.xp, 10);
  s.damageTank(enemy, 9999, ally.id, ally.team);
  assert.equal(ally.xp, hp - 30 + 50);
  assert.equal(ally.kills, 1);
...
test("player and bots promote at exact thresholds, preserve hull percentage, and cap at Heroic", () => {
  ...
  for (const t of [player, ally]) {
    ...
    earnExperience(s, t, RANKS[1].xp - 0.5);
    assert.equal(rankIndex(t), 0);
    earnExperience(s, t, 0.5);
    assert.equal(rankIndex(t), 1);
    near(t.hp, s.maxHealth(t) * 0.4);
    earnExperience(s, t, RANKS[2].xp - RANKS[1].xp);
    assert.equal(rankIndex(t), 2);
    earnExperience(s, t, RANKS[3].xp - RANKS[2].xp);
    assert.equal(rankIndex(t), 3);
    earnExperience(s, t, 99999);
    assert.equal(t.xp, RANKS[3].xp);
    near(t.hp, s.maxHealth(t) * 0.4);
...
test("one large XP award reaches the correct rank and emits one promotion", () => {
  ...
  assert.match(s.events[0].label!, /ELITE/);
  s.dispose();
});
...
test("all five weapons snapshot rank damage; human and bot reload bonuses stack with rapid fire", () => {
  ...
  for (const t of [player, ally])
    for (const weapon of AMMO_ORDER) {
      ...
      const rookie = weaponInterval(t),
        botRookie = botReload(t, 0, weapon);
      t.xp = RANKS[3].xp;
      t.rapid = 12;
      t.cooldown = 0;
      ...
      t.xp = 0;
      near(s.shots[0].damage, WEAPONS[weapon].damage * 1.3);
      t.rapid = 0;
    }
  s.dispose();
});
...
test("promotion scales a pending cannon reload and the bot decision timer", () => {
  const { s, ally } = fixture();
  ally.cooldown = 0.6;
  ally.brain.fireDelay = 2;
  earnExperience(s, ally, RANKS[1].xp);
  near(ally.cooldown, 0.6 / 1.1);
  near(ally.brain.fireDelay, 2 / 1.1);
  s.dispose();
});
...
test("mines snapshot damage and destroyed owners or replacements cannot gain XP from old ordnance", () => {
  for (const respawn of [false, true]) {
    ...
    player.xp = RANKS[3].xp;
    placeMine(s, player);
    const mine = s.mines[0];
    assert.equal(mine.damage, 130);
    assert.equal(mine.ownerLife, 0);
    ...
    const fresh = s.addTank(enemy.team, false, "balanced", 0);
    move(fresh, 0, 10);
    fresh.protection = 0;
    s.world.step();
    s.shots.push({
    ...
    assert.equal(fresh.hp, s.maxHealth(fresh) - 30);
    assert.equal(player.xp, xp);
    s.dispose();
  }
});
...
test("mine and drum chains retain the initiating tank's XP and life attribution", () => {
  for (const oldLife of [false, true]) {
    ...
    assert.equal(enemy.alive, false);
    assert.equal(player.xp, oldLife ? 0 : 60);
...
test("Elite and Heroic repair only after five quiet seconds; shield hits and firing delay repair", () => {
  ...
  s.damageTank(player, 20, -1, enemy.team);
  assert.equal(player.lastCombat, 5);
  s.elapsed = 9.9;
  repairVeteran(s, player, 1);
  assert.equal(player.hp, 30);
  s.elapsed = 11;
  player.cooldown = 0;
  s.step({ ...idleCommand(), fire: true });
  assert.equal(player.hp, 30);
  const time = s.elapsed;
  s.match.phase = "paused";
  for (let i = 0; i < 400; i++) s.step();
  assert.equal(s.elapsed, time);
  assert.equal(player.hp, 30);
  s.start();
  s.shots = [];
  for (let i = 0; i < 361; i++) s.step();
  assert.ok(player.hp > 32 && player.hp < 33);
  player.hp = s.maxHealth(player) - 0.01;
  repairVeteran(s, player, 1);
  assert.equal(player.hp, s.maxHealth(player));
  s.damageTank(player, 9999, player.id, player.team);
  repairVeteran(s, player, 10);
  ...
  for (const solo of [false, true])
    for (const t of [player, enemy]) {
      ...
      for (const kind of ["scout", "balanced", "heavy"] as VehicleKind[]) {
        ...
        t.hp = 1;
        const expected =
          Math.round(VEHICLES[kind].health * (solo && t === enemy ? 0.4 : 1) * 1.2 * 100) / 100;
...
test("death stops XP, respawn resets rank before computing new chassis hull, and round reset clears all", () => {
  const { s, player, ally } = fixture();
  for (const t of [player, ally]) {
    earnExperience(s, t, RANKS[3].xp);
    s.damageTank(t, 9999, t.id, t.team);
    earnExperience(s, t, 30);
    assert.equal(t.xp, RANKS[3].xp);
    s.humanKind = "heavy";
    s.respawn(t);
    assert.equal(t.xp, 0);
  ...
  assert.equal(snapshot.tanks[0].rank, 2);
  assert.equal(snapshot.tanks[0].maxHp, 161);
  snapshot.tanks[0].xp = 0;
  assert.equal(player.xp, RANKS[2].xp);
  s.reset();
  assert.ok(s.tanks.every((t) => t.xp === 0 && rankIndex(t) === 0));
  s.dispose();
});
