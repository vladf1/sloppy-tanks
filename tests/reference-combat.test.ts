...
function shot(s: Simulation, x: number, z: number, vx: number, vz: number, team: Team): Shot {
  return {
    ...
    vz,
    team,
    owner: s.tanks.find((t) => t.team === team)?.id ?? team,
    damage: 40,
    bounces: 0,
    life: 3.5,
    piercing: 0,
...
test("movement adds 20% to V-Tanks-scaled speeds, preserves class ratios and normalizes diagonals", () => {
  ...
  for (const diagonal of [false, true]) {
    ...
    for (let i = 0; i < boostSteps; i++)
      s.step({ ...idleCommand(), moveX: diagonal ? 1 : 0, moveZ: 1 });
    assert.ok(
      Math.abs(Math.hypot(t.body.linvel().x, t.body.linvel().z) / VEHICLES[t.kind].speed - 1.5) <
        0.03,
    );
    s.dispose();
  }
...
test("tracks are distance-spaced at 30/120 FPS, skip stationary tanks and teleports, and stay bounded across resets", () => {
  ...
  for (const fps of [30, 120]) {
    ...
    for (let i = 0; i <= fps * 2; i++) {
      const x = (i * 6) / fps;
      t.body.setTranslation({ x, y: 0.65, z: 0 }, true);
      t.previous = { x, z: 0 };
      s.elapsed = i / fps;
      tracks.update(s, 1);
    }
    ...
    tracks.reset();
    assert.equal(tracks.mesh.count, 0);
    tracks.update(s, 1);
    assert.equal(tracks.mesh.count, 0);
    tracks.dispose();
    s.dispose();
...
test("four repair pickups are symmetric, clear of cover, and away from spawn pads", () => {
  ...
  for (const p of repairs) {
    ...
    assert.ok(
      s.covers.every((c) => Math.abs(p.x - c.x) > c.w / 2 + 2 || Math.abs(p.z - c.z) > c.d / 2 + 2),
    );
    for (const spawn of [...spawnPositions(0), ...spawnPositions(1)])
      assert.ok(
        Math.hypot(p.x - spawn.x, p.z - spawn.z) >= 8,
