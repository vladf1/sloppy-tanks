...
import { idleCommand, type Shot, type Weapon, type Team, type SpecialAmmo } from "../src/game/types";
...
function arena(count = 1) {
  ...
  s.covers = []; s.coverByCollider.clear(); s.pickups = []; s.nav.rebuild([]);
  ...
  for (const [i, t] of s.tanks.entries()) {
    t.human = true; t.protection = 0; t.aim = 0;
...
function shot(s: Simulation, weapon: Weapon, x: number, vx: number, team: Team): Shot {
  return { id: s.nextId++, weapon, x, z: 0, vx, vz: 0, team, owner: 999 + team,
    damage: WEAPONS[weapon].damage, bounces: 0, life: 3.5, piercing: weapon === "piercing" ? 1 : 0 };
}

for (const weapon of AMMO_ORDER) test(`${weapon} emits the correct shot, costs one unit, and respects cooldown`, () => {
  ...
  for (const p of s.shots) {
    assert.equal(p.weapon, weapon); assert.equal(p.damage, WEAPONS[weapon].damage);
  ...
  const after = { ...t.ammo }, cooldown = t.cooldown, count = s.shots.length;
  fireWeapon(s, t);
  assert.equal(s.shots.length, count); assert.deepEqual(t.ammo, after);
  assert.equal(t.cooldown, cooldown);
  t.cooldown = 0; t.rapid = 12; fireWeapon(s, t);
...
test("standard remains unlimited over sustained firing", () => {
  const s = arena(), t = s.human;
  for (let i = 0; i < 500; i++) { t.cooldown = 0; fireWeapon(s, t); s.shots = []; }
  assert.equal(s.shotsFired, 500); assert.deepEqual(t.ammo, emptyAmmo());
  assert.equal(t.selectedAmmo, "standard"); s.dispose();
});

test("selection cycles both directions, skips empty slots, wraps, and never changes cooldown", () => {
  const s = arena(), t = s.human;
  t.cooldown = 0.72;
  selectAmmo(t, 1); assert.equal(t.selectedAmmo, "standard");
  t.ammo.spread = 2; t.ammo.piercing = 1;
  for (const expected of ["spread", "piercing", "standard"]) {
    selectAmmo(t, 1); assert.equal(t.selectedAmmo, expected);
  }
  for (const expected of ["piercing", "spread", "standard"]) {
    selectAmmo(t, -1); assert.equal(t.selectedAmmo, expected);
  }
  selectAmmo(t, "rocket"); assert.equal(t.selectedAmmo, "standard");
  assert.equal(t.cooldown, 0.72); s.dispose();
});

test("selection precedes held fire, depletion falls back, and switching cannot bypass reload", () => {
  const s = arena(), t = s.human;
  t.ammo.rocket = 1; t.ammo.spread = 2;
  s.step({ ...idleCommand(), fire: true, ammoSelection: "rocket" });
  assert.equal(s.shots[0].weapon, "rocket"); assert.equal(t.ammo.rocket, 0);
  ...
  assert.equal(s.shotsFired, 1); assert.equal(t.ammo.spread, 2);
  ...
  assert.equal(t.cooldown, WEAPONS.standard.interval / 1.2); s.dispose();
});
...
test("death, respawn and reset clear inventories; snapshots own their inventory copy", () => {
  ...
  assert.deepEqual(t.ammo, emptyAmmo()); assert.equal(t.selectedAmmo, "standard");
  assert.equal(s.pickups.length, 0); assert.equal(snapshot.ammo.piercing, 24);
  t.ammo.spread = 1; selectAmmo(t, 1); assert.equal(t.selectedAmmo, "standard");
  s.respawn(t); assert.deepEqual(t.ammo, emptyAmmo());
  t.ammo.rocket = 7; t.selectedAmmo = "rocket";
  s.reset(); assert.ok(s.tanks.every(t => t.selectedAmmo === "standard"));
  assert.ok(s.tanks.every(t => Object.values(t.ammo).every(n => n === 0))); s.dispose();
});

test("paused and results simulations ignore selection", () => {
  const s = arena(), t = s.human; t.ammo.spread = 1;
  for (const phase of ["ready", "paused", "results"] as const) {
    s.match.phase = phase; s.step({ ...idleCommand(), ammoSelection: 1, fire: true });
    assert.equal(t.selectedAmmo, "standard"); assert.equal(t.ammo.spread, 1);
  }
  s.dispose();
});

for (const kind of AMMO_ORDER.filter(isSpecialAmmo)) test(`${kind} crates cap stock and report actual receipt without equipping or clearing cooldown`, () => {
  const s = arena(), t = s.human;
  t.cooldown = 0.6;
  const p = crate(s, kind);
  assert.equal(collectPickup(s, t, p), true);
  assert.equal(t.ammo[kind], WEAPONS[kind].perCrate);
  assert.equal(t.selectedAmmo, "standard"); assert.equal(t.cooldown, 0.6);
  ...
  const full = crate(s, kind), count = s.events.length;
  assert.equal(collectPickup(s, t, full), false); assert.equal(full.available, true);
  assert.equal(full.cooldown, 0); assert.equal(s.events.length, count); s.dispose();
});

test("simultaneous collection skips full tanks, awards one recipient, and refills after 13 seconds", () => {
  const s = arena(3), [full, first, second] = s.tanks;
  for (const [i, t] of s.tanks.entries()) {
  ...
  full.ammo.rocket = 24;
  const p = crate(s, "rocket"); s.pickups.push(p);
  s.step();
  ...
  assert.equal(p.available, true); s.dispose();
});

for (const opposing of ["standard", "rocket", "piercing"] as const)
  for (const reversed of [false, true]) test(`piercing intercepts ${opposing}, order=${reversed}`, () => {
    const s = arena(0), a = shot(s, "piercing", -2, 20, 0), b = shot(s, opposing, 2, -20, 1);
    ...
    assert.equal(s.shots.length, opposing === "piercing" ? 2 : 1); s.dispose();
  });

test("a spent piercing shell uses normal interception and opposing damage ownership", () => {
  const s = arena(2), [aTank, bTank] = s.tanks;
  aTank.body.setTranslation({ x: 5, y: 0.65, z: -2.6 }, true);
  bTank.body.setTranslation({ x: 5, y: 0.65, z: 2.6 }, true);
  aTank.hp = bTank.hp = 40; s.world.step();
  const a = shot(s, "piercing", 0, 20, 0); a.owner = aTank.id;
  const first = shot(s, "rocket", 2, 0, 1); first.owner = bTank.id;
  const second = shot(s, "standard", 6, 0, 1); second.owner = bTank.id;
  s.shots = [second, first, a]; stepProjectiles(s, 0.4);
  assert.equal(s.shots.length, 0);
  assert.equal(s.events.filter(e => e.type === "explosion").length, 1);
  assert.equal(aTank.alive, false); assert.equal(bTank.alive, false);
  assert.equal(aTank.kills, 1); assert.equal(bTank.kills, 1); s.dispose();
});

test("piercing stops on tanks and cover and cannot intercept through thin cover", () => {
  const s = arena();
  s.human.body.setTranslation({ x: 2, y: 0.65, z: 0 }, true); s.world.step();
  const hp = s.human.hp;
  s.shots = [shot(s, "piercing", -2, 20, 1)]; stepProjectiles(s, 0.3);
  assert.equal(s.shots.length, 0); assert.equal(s.human.hp, hp - 40);
  s.addCover({ kind: "wall", x: 0, z: 0, w: 0.1, d: 10, h: 3, hp: Infinity, color: 0 });
  s.world.step(); s.events = [];
  ...
  assert.ok(s.events.every(e => e.size === 0.6), "only cover impacts, no shell interception");
  s.dispose();
});

test("Pine Village has eight mirrored ammo crates with navigable approaches from both teams", () => {
  const s = new Simulation(123), ammo = s.pickups.filter(p => isSpecialAmmo(p.kind));
  assert.equal(ammo.length, 8);
  for (const kind of AMMO_ORDER.filter(isSpecialAmmo)) assert.equal(ammo.filter(p => p.kind === kind).length, 2);
  for (const p of ammo) {
    ...
    for (const team of [0, 1] as const) for (const spawn of spawnPositions(team)) {
...
test("bot roles select stocked ammo, use standard on cover, and fall back after depletion", () => {
  const s = arena(2), [bot, enemy] = s.tanks;
  bot.human = false; enemy.human = true;
  bot.ammo = { spread: 5, rocket: 5, ricochet: 5, piercing: 5 };
  for (const role of Object.keys(BOT_AMMO) as (keyof typeof BOT_AMMO)[]) {
    bot.brain.personality = role; bot.brain.decision = 0;
  ...
  bot.brain.target = 0; bot.brain.memory = 0; bot.brain.decision = 99;
  ...
  bot.ammo = emptyAmmo(); bot.ammo.rocket = 1; bot.selectedAmmo = "rocket";
  fireWeapon(s, bot); assert.equal(bot.selectedAmmo, "standard");
  bot.cooldown = 0; fireWeapon(s, bot); assert.equal(s.shots.at(-1)!.weapon, "standard");
  s.dispose();
});

test("bots drive to useful crates, consume ammo and ignore a full crate", () => {
  const s = arena(), bot = s.human;
  bot.human = false; bot.brain.personality = "artillery";
  const p = crate(s, "rocket"); p.z = 6; s.pickups.push(p);
  bot.brain.decision = 0; botCommand(s, bot, STEP);
  assert.equal(bot.brain.mode, "pickup");
  for (let i = 0; i < 180 && p.available; i++) s.step();
  assert.equal(p.available, false); assert.equal(bot.ammo.rocket, 12);
  selectAmmo(bot, preferredAmmo(bot)); fireWeapon(s, bot); assert.equal(bot.ammo.rocket, 11);
  bot.ammo.rocket = 24; p.available = true; bot.brain.decision = 0;
  botCommand(s, bot, STEP); assert.notEqual(bot.brain.mode, "pickup");
  s.dispose();
});
