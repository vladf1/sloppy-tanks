...
/**
 * A HMMWV at the origin facing north and one enemy `range` metres ahead on an
 * otherwise empty map. Cover is added before navigation is rebuilt.
 */
function duel(range: number, covers: Parameters<Simulation["addCover"]>[0][] = []) {
  ...
  clearArena(simulation, [hunter, target]);
  for (const cover of covers) simulation.addCover(cover);
  simulation.nav.rebuild(simulation.covers);
  hunter.body.setTranslation({ x: 0, y: 0.65, z: 0 }, true);
  target.body.setTranslation({ x: 0, y: 0.65, z: range }, true);
  hunter.previous = { x: 0, z: 0 };
  target.previous = { x: 0, z: range };
  hunter.aim = 0;
  hunter.heading = 0;
  hunter.cooldown = 0;
  hunter.protection = 0;
  ...
  return { simulation, hunter, target };
}

test("team rounds include TOW HMMWVs while solo rounds do not", () => {
...
test("a HMMWV fires an unlimited TOW that leaves a fresh Bruiser alive", () => {
  const { simulation, hunter, target } = duel(12);
  try {
...
test("HMMWVs drive forward except during recovery and guide TOWs with a capped turn", () => {
  const { simulation, hunter, target } = duel(24);
  try {
    hunter.brain.target = target.id;
    simulation.start();
...
test("HMMWVs never spend TOWs on cover or hidden tanks", () => {
  for (const kind of ["tree", "timber"] as const) {
    const { simulation, hunter, target } = duel(10, [
      { kind, x: 0, z: 5, w: 2, d: 2, h: kind === "tree" ? 6 : 2, hp: 80, color: 0 },
    ]);
    try {
      hunter.brain.target = target.id;
...
test("Humvees plan an escape, fire once, withdraw and wait before attacking again", () => {
  const { simulation, hunter, target } = duel(28);
  try {
    hunter.brain.target = target.id;
...
test("Humvees prefer a concealed escape and do not escort idle allies", () => {
  const { simulation, hunter, target } = duel(28, [
    { kind: "concrete", x: 7, z: 3, w: 5, d: 2, h: 3, hp: Infinity, color: 0 },
  ]);
  try {
    hunter.brain.decision = 0;
    botCommand(simulation, hunter, STEP);
