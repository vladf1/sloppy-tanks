  ...
  for (const kind of ["scout", "balanced", "heavy"] as const)
    for (const heading of [0, Math.PI / 2, Math.PI, -Math.PI / 2]) {
      ...
      try {
        ...
        assert.ok(
          v.x * Math.sin(heading) + v.z * Math.cos(heading) > 0,
...
test("perpendicular input gradually turns from rest without strafing, then reaches full speed", () => {
  for (const heading of [0, Math.PI / 2, Math.PI, -Math.PI / 2])
    for (const side of [-1, 1]) {
      const s = arena("balanced", heading),
        t = s.human,
        target = heading + (side * Math.PI) / 2;
      try {
        drive(s, target, 1);
        assert.ok(Math.abs(angleDelta(heading, t.heading)) < 0.07);
        assert.ok(
          angleDelta(heading, t.heading) * side > 0,
          "broadside input favors forward steering",
        );
        ...
        const v = t.body.linvel();
        assert.ok(
          Math.abs(v.x * Math.cos(t.heading) - v.z * Math.sin(t.heading)) < 0.05,
        ...
        assert.ok(
          Math.hypot(t.body.linvel().x, t.body.linvel().z) > VEHICLES.balanced.speed * 0.98,
        );
...
test("a diagonal behind the hull makes a short reverse turn across the angle wrap", () => {
  const s = arena("balanced", Math.PI - 0.1),
    t = s.human;
  try {
