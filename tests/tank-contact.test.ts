        ...
        for (const team of [0, 1] as const) {
          ...
          for (const [i, t] of s.tanks.entries()) {
            ...
            t.body.setTranslation(
              { x: direction.x * side * 4, y: 0.65, z: direction.z * side * 4 },
              true,
            );
            t.body.setRotation(
          ...
          for (let frame = 0; frame < 120; frame++) {
            for (const [i, t] of s.tanks.entries()) {
              ...
              const dx = direction.x * speed - v.x,
                dz = direction.z * speed - v.z;
              const amount = Math.min(1, (MOVE_ACCELERATION * STEP) / (Math.hypot(dx, dz) || 1));
              t.body.applyImpulse(
                { x: dx * amount * t.body.mass(), y: 0, z: dz * amount * t.body.mass() },
                true,
              );
            }
            s.world.timestep = STEP;
            s.world.step();
          ...
          assert.ok(
            ...
            `${kind} ${axis} ${angle}: ${minimum} versus ${expected}`,
          );
          s.dispose();
        }
});
...
test("model-sized contact collider is recreated on class-changing respawn and cleaned on death", () => {
  ...
  const restitution = t.collider.restitution(),
    friction = t.collider.friction();
  t.protection = 0;
  ...
  for (const aKind of ["scout", "balanced", "heavy"] as const)
    for (const bKind of ["scout", "balanced", "heavy"] as const) {
      ...
      for (const [i, t] of s.tanks.entries()) {
        ...
        t.body.setRotation(
          { x: 0, y: Math.sin((i * Math.PI) / 4), z: 0, w: Math.cos((i * Math.PI) / 4) },
          true,
        );
        t.body.lockRotations(true, true);
      }
      ...
      for (let frame = 0; frame < 180; frame++) {
        for (const [i, t] of s.tanks.entries()) {
          ...
          const change = Math.max(
            -MOVE_ACCELERATION * STEP,
            Math.min(MOVE_ACCELERATION * STEP, target - t.body.linvel().z),
          );
  ...
  for (const kind of ["scout", "balanced", "heavy"] as const)
    for (const side of [-1, 1]) {
      ...
      for (let frame = 0; frame < 180; frame++) {
        const change = Math.max(
          -MOVE_ACCELERATION * STEP,
          Math.min(MOVE_ACCELERATION * STEP, VEHICLES[kind].speed * side * 1.5 - t.body.linvel().z),
        );
        t.body.applyImpulse({ x: 0, y: 0, z: change * t.body.mass() }, true);
        s.world.timestep = STEP;
        s.world.step();
        assert.ok(
          t.body.translation().z * side + reach <= -0.25 + 0.015,
          `${kind}/${side} clipped wall`,
        );
      }
      s.dispose();
    }
