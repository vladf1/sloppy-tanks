...
test("room listings sort by players, expire, and disappear with the last connected player", () => {
  const catalog = new RoomCatalog();
  catalog.update(entry, 0);
  catalog.update({ ...entry, room: "BCDEFGHJ", players: 2, reserved: 2 }, 1);
  assert.deepEqual(
    catalog.list(2).map((room) => room.players),
    [2, 1],
  );
  catalog.update({ ...entry, players: 0 }, 3);
  assert.deepEqual(
    catalog.list(3).map((room) => room.room),
    ["BCDEFGHJ"],
  );
  assert.equal(catalog.list(ROOM_LIST_TTL_MS).length, 1);
  assert.equal(catalog.list(ROOM_LIST_TTL_MS + 1).length, 0);
});
