...
test("bots brake and settle near a destination without repeated direction flips, including double speed", () => {
  for (const personality of ["scout", "guard", "heavy"] as const)
    for (const speed of [1, 2]) {
      const s = arena(),
        bot = s.addTank(0, false, "balanced", personality === "heavy" ? 3 : 1);
      bot.brain.personality = personality;
      place(bot, 0, 0);
...
test("a retreating guard skirts a wall instead of alternating attack and retreat", () => {
  const s = arena(),
    ...
    enemy = s.addTank(1, true, "balanced");
  place(bot, 0, -8);
  place(enemy, 0, 0);
...
test("head-on allies pass each other and both reach their destinations", () => {
  ...
  assert.ok(distance(b.body.translation(), b.brain.goal) < 0.5);
  s.dispose();
});
...
test("a stalled bot commits to its recovery route across combat decisions and clears it on respawn", () => {
  ...
  assert.ok(bot.brain.recovery > 1);
  ...
  bot.protection = 0;
  s.damageTank(bot, 999, enemy.id, enemy.team);
  s.respawn(bot);
  assert.equal(bot.brain.recovery, 0);
  assert.equal(bot.brain.avoidanceTime, 0);
  assert.equal(bot.brain.stuck, 0);
  assert.equal(bot.brain.recoveries, 0);
  s.dispose();
});
...
test("bots retain comparable visible targets but react to a substantially closer enemy", () => {
  const s = arena(),
    bot = s.addTank(0, false, "balanced", 1);
  const a = s.addTank(1, true, "balanced"),
    b = s.addTank(1, true, "balanced");
  place(bot, 0, 0);
  place(a, 2, 18);
  place(b, -2, 19);
  s.world.step();
  bot.brain.decision = 0;
  botCommand(s, bot, STEP);
  assert.equal(bot.brain.target, a.id);
  place(b, -2, 17);
  s.world.step();
  bot.brain.decision = 0;
  botCommand(s, bot, STEP);
  ...
  bot.brain.decision = 0;
  botCommand(s, bot, STEP);
  assert.equal(bot.brain.target, b.id);
  s.dispose();
});
...
test("pickup and patrol destinations persist across decisions and unavailable crates are abandoned", () => {
  ...
  place(bot, 20, -38);
  s.world.step();
  bot.brain.goal = { x: 20, z: -38 };
  bot.brain.decision = 0;
  botCommand(s, bot, STEP);
  const patrol = { ...bot.brain.goal };
  assert.equal(patrol.x, 46);
  for (let i = 0; i < 5; i++) {
    bot.brain.decision = 0;
    botCommand(s, bot, STEP);
    assert.deepEqual(bot.brain.goal, patrol);
  }
  s.pickups = [
    { id: 999, kind: "rocket", available: true, cooldown: 0, x: 24, z: -38 },
  ...
  bot.brain.decision = 0;
  botCommand(s, bot, STEP);
  assert.equal(bot.brain.pickupTarget, 1000);
  s.dispose();
});
