...
for (const kind of Object.keys(PICKUPS) as PickupKind[]) {
  ...
  if (kind === "speed")
    ...
    c.fillRect(109, 57, 38, 124);
    c.fillRect(66, 100, 124, 38);
  } else if (kind === "shield") {
    line([
      [128, 52],
      [181, 73],
...
// Neutral paint wear multiplies team paint without introducing another hue.
{
  ...
  for (let i = 0; i < 100; i++) {
    ...
    for (const ox of [-size, 0, size])
      for (const oy of [-size, 0, size]) {
        c.lineWidth = width;
        c.strokeStyle = i < 24 ? "#aaaaaa" : "#bcbcbc";
        c.beginPath();
        c.moveTo(x + ox, y + oy);
        c.lineTo(x + dx + ox, y + dy + oy);
        c.stroke();
        c.lineWidth = Math.max(1, width * 0.4);
        c.strokeStyle = "#ffffff";
        c.beginPath();
        c.moveTo(x + ox, y + oy + width / 2);
        c.lineTo(x + dx + ox, y + dy + oy + width / 2);
        c.stroke();
      }
  }
