...
try {
  ...
  for (const { kind, hp, hit, destroyed } of results) {
    ...
    assert.equal(hit.hp, hp - 20);
    assert.equal(hit.impacts, 1);
    assert.equal(hit.destroys, 0);
    assert.ok(hit.chips > 0);
    assert.equal(hit.leaves > 0, kind === "tree");
