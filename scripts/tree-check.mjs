...
try {
  ...
  const result = await page.evaluate(async () => {
    const { treeModel, setTreeDestroyed, TREE_FAMILIES } = await import("/sloppy-tanks/src/game/tree-models.ts");
    ...
    s.human.previous = { x: first.x, z: first.z + 5 }; v.zoom = 23;
    ...
    const scene = new THREE.Scene(); scene.background = new THREE.Color(0xaac8cf);
    scene.add(new THREE.HemisphereLight(0xf4f5de, 0x516044, 2.3));
    const sun = new THREE.DirectionalLight(0xffebc5, 3); sun.position.set(-8, 20, 12); scene.add(sun);
    const ground = new THREE.Mesh(new THREE.PlaneGeometry(42, 24), new THREE.MeshStandardMaterial({ color: 0x9da77c, roughness: 1 }));
    ground.rotation.x = -Math.PI / 2; ground.position.y = -0.015; scene.add(ground);
    const specimens = [];
    for (const [i, family] of TREE_FAMILIES.entries()) {
      const c = all.get(family); const tree = treeModel(c), stump = treeModel(c);
      setTreeDestroyed(stump, true);
      tree.position.set((i - 2.5) * 5.3, 0, -2.2);
      stump.position.set((i - 2.5) * 5.3, 0, 3.7);
      scene.add(tree, stump); specimens.push(tree, stump);
      const label = document.createElement("div"); label.textContent = family;
      label.style.cssText = `position:fixed;left:${(i + .5) / 6 * 100}%;bottom:105px;transform:translateX(-50%);font:600 20px system-ui;color:#172c26;pointer-events:none`;
      label.className = "tree-label"; document.body.append(label);
    }
    const camera = new THREE.PerspectiveCamera(40, 1.6, 0.1, 100);
    camera.position.set(0, 15, 28); camera.lookAt(0, 2.1, 0);
  ...
  await page.waitForFunction(() => {
    let ready = true;
    window.treeCheck.scene.traverse(m => { if (m.material?.map) ready &&= !!m.material.map.image?.complete; });
    return ready;
  });
  await page.evaluate(() => {
  ...
  const destroyed = await page.evaluate(() => {
    ...
    return {
      ...
      sameStump: before.every((g, i) => g.getObjectByName("rooted-stump") === stumps[i] && stumps[i].children.map(m => m.geometry.uuid).join() === signatures[i]),
      onlyStumps: before.every(g => g.visible && !g.userData.crown.visible && g.userData.cutSurface.visible),
      hasBurst: s.fragments.length > fragments && v.particles.length > particles &&
        v.particles.some(p => p.shape === "leaf") && v.particles.some(p => p.shape === "splinter"),
      longerDebris: s.fragments.every(f => f.shape === "wood" && f.life >= 4.8 && f.life <= 7.8) &&
