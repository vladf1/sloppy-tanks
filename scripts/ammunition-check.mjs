...
try {
  ...
  const advance = () =>
    page.evaluate(() => {
      for (let i = 0; i < 4; i++) window.advanceFrame(17);
    });
  ...
  await page.evaluate(() => {
    window.sloppy.controls.lastAmmoScroll = performance.now();
  });
  await page.mouse.wheel(0, 80);
  await page.mouse.wheel(0, 80);
  await advance();
  ...
  await page.evaluate(() => {
    ...
    t.ammo.spread = 1;
    t.cooldown = 0;
    window.sloppy.sim.shots = [];
    window.sloppy.sim.shotsFired = 0;
  ...
  await page.evaluate(() => {
    for (let i = 0; i < 60; i++) window.advanceFrame(17);
  });
  await page.mouse.up();
  ...
  await page.evaluate(() => {
    const d = window.sloppy,
      t = d.sim.human;
    t.protection = 0;
    d.sim.damageTank(t, 999, t.id, t.team);
  });
  await page.mouse.wheel(0, 80);
  assert.equal(await page.evaluate(() => window.sloppy.controls.ammoSelection), undefined);
  assert.equal(await selected(), "standard");
  await page.evaluate(() => {
    window.sloppy.restart();
  });
  assert.equal(await page.evaluate(() => window.sloppy.controls.ammoSelection), undefined);
  console.log(
    "Real wheel, Shift-wheel limits, throttling, held fire, depletion, mines, pause, death and reset passed.",
  );
  ...
  for (const scenario of ["standard", "plentiful-spread-piercing"])
    for (const seed of [12345, 45678, 98765]) {
        ...
        ({ scenario, seed }) => {
          ...
          sim.step = (...args) => {
            for (const [i, t] of sim.tanks.entries()) {
              ...
              t.ammo = {
                spread: scenario === "standard" ? 0 : 36,
                piercing: scenario === "standard" ? 0 : 48,
                rocket: 0,
                ricochet: 0,
              };
      ...
      console.log(
        JSON.stringify({
          ...
          simulationP95: report.simulationP95,
    ...
    JSON.stringify(
      {
        ...
        resolution: [1600, 900],
        workload:
          "24 tanks, rapid fire, 5 seconds warmup + 10 seconds measured; standard vs continually refilled spread/piercing; three seeds",
        input:
          "real wheel, Shift-wheel limits, throttle, held fire, depletion, mines, pause, death, reset and narrow HUD passed",
        errors,
        runs,
      },
      null,
