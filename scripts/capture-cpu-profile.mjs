...
for (let i = 0; i < (profile.samples ?? []).length; i++) {
  ...
  weights.set(id, (weights.get(id) ?? 0) + (profile.timeDeltas?.[i] ?? 1000));
}
const rows = [...weights]
  .map(([id, us]) => ({
    ms: +(us / 1000).toFixed(1),
    fn: nodes.get(id).callFrame.functionName,
    url: nodes.get(id).callFrame.url,
    line: nodes.get(id).callFrame.lineNumber + 1,
  }))
  .sort((a, b) => b.ms - a.ms)
  .slice(0, 30);
await writeFile(`artifacts/profiles/${label}-top.json`, JSON.stringify(rows, null, 2));
console.log(JSON.stringify(rows.slice(0, 18), null, 2));
