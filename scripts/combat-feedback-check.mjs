...
try {
  ...
  checks.push(
    "Real Q/E, 1–5, wrap, empty-slot rejection, cooldown preservation, ready/dim reticle and all five shot sounds",
  );
  ...
  await page.evaluate(() => {
    ...
    target.previous = { x: 0, z: 33 };
    target.protection = 0;
    target.brain.decision = 100;
    target.brain.goal = { x: 0, z: 33 };
    d.view.reset(s);
    s.world.step();
    window.targetId = target.id;
  ...
  await page.evaluate(() => {
    const d = window.sloppy,
      s = d.sim,
      t = s.human,
      target = s.tanks.find((t) => t.id === window.targetId);
    d.audio.lastHit = -Infinity;
    window.soundCalls = [];
    t.cooldown = 0.5;
    s.damageTank(target, 10, t.id, t.team);
    s.damageTank(target, 10, t.id, t.team);
    window.advanceFrame(17);
  });
  ...
  await page.evaluate(() => {
    ...
    d.audio.lastHit = -Infinity;
    window.soundCalls = [];
    s.damageTank(target, 999, t.id, t.team);
    window.advanceFrame(17);
  ...
  assert.ok((await page.evaluate(() => window.soundCalls)).includes("hit"));
  checks.push(
    "Surviving and lethal credited hits flash/tick; self, other shooter and cover do not; pellet ticks coalesce; flash expires",
  );
  // Pause owns both shortcuts and the existing status panel's animation.
  await page.evaluate(() => {
    window.sloppy.sim.human.hp = 25;
  });
  await advance();
  assert.ok(
    !(await page.locator(".status").evaluate((e) => e.classList.contains("critical-health"))),
  );
  await page.evaluate(() => {
    window.sloppy.sim.human.hp = 24;
  });
  await advance();
  ...
  await page.keyboard.press("1");
  await advance();
  assert.equal(await selected(), pausedSelection);
  assert.equal(
    await page.locator(".status").evaluate((e) => getComputedStyle(e).animationPlayState),
    "paused",
  );
  ...
  await page.evaluate(() => {
    window.sloppy.sim.human.hp = 100;
  });
  await advance();
  assert.ok(
    !(await page.locator(".status").evaluate((e) => e.classList.contains("critical-health"))),
  ...
  await page.keyboard.press("Escape");
  await advance();
  const pausedPads = await pads();
  await advance(60);
  assert.deepEqual(await pads(), pausedPads);
  await page.locator("#resume").click();
  await advance(780);
  ...
  const geometries = await page.evaluate(() => {
    ...
    for (let i = 0; i < 10; i++) {
      d.restart();
      window.advanceFrame(17);
      counts.push(d.view.renderer.info.memory.geometries);
    }
    return counts;
  });
  ...
  perf.on("pageerror", (e) => errors.push(e.message));
  await perf.goto(`${url}?autoplay`);
  await perf.waitForFunction(() => !!window.sloppy);
  await perf.evaluate(() => {
    const d = window.sloppy;
    d.sim.seed = 12345;
    d.sim.roundCount = 24;
  ...
  console.log(
    JSON.stringify({ checks, fps: report.fps, frameP99: report.frameP99, errors }, null, 2),
  );
} finally {
  await browser.close();
}
