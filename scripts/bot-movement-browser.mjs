...
try {
  ...
  for (const scenario of ["retreat-at-wall", "head-on-allies"]) {
    ...
    await page.evaluate(scenario => {
      ...
      s.humanTeam = 1;
      for (const [t, x, z] of [[bot, 0, scenario === "head-on-allies" ? -6 : -8], [other, 0, 6], [player, 0, 0]]) {
        t.body.setTranslation({ x, y: 0.65, z }, true);
        t.previous = { x, z }; t.brain.last = { x, z }; t.protection = 1000;
      }
      if (scenario === "retreat-at-wall") {
        s.world.removeRigidBody(other.body); s.tanks.splice(s.tanks.indexOf(other), 1);
        s.addCover({ kind: "concrete", x: 0, z: -12, w: 16, d: 2, h: 3, hp: Infinity, color: 0xc4d4da });
      } else {
        // The observer renders the scene but does not obstruct the two bots.
        for (let i = 0; i < player.body.numColliders(); i++) player.body.collider(i).setCollisionGroups(0);
        for (const [t, z] of [[bot, 16], [other, -16]]) {
          t.brain.decision = 999; t.brain.goal = { x: 0, z };
        }
      }
      s.nav.rebuild(s.covers);
      if (scenario === "head-on-allies") for (const t of [bot, other]) t.brain.path = s.nav.find(t.previous, t.brain.goal);
      s.world.step(); d.view.reset(s); d.view.zoom = 29;
      const step = s.step.bind(s), last = new Map(), flips = new Map();
      window.botCheck = () => s.tanks.filter(t => !t.human).map(t => ({
        x: t.body.translation().x, z: t.body.translation().z,
        goal: t.brain.goal, reversals: flips.get(t.id) ?? 0,
      }));
      s.step = (...args) => {
        step(...args);
        for (const t of s.tanks.filter(t => !t.human)) {
          const c = t.command, length = Math.hypot(c.moveX, c.moveZ);
          if (length < 0.15) continue;
          const v = [c.moveX / length, c.moveZ / length], previous = last.get(t.id);
          if (previous && previous[0] * v[0] + previous[1] * v[1] < -0.5) flips.set(t.id, (flips.get(t.id) ?? 0) + 1);
          last.set(t.id, v);
        }
      };
    }, scenario);
  ...
  await page.goto(`${url}?autoplay`); await page.waitForFunction(() => !!window.sloppy);
  await page.evaluate(() => {
    const d = window.sloppy; d.sim.seed = 12345; d.sim.roundCount = 24;
    d.sim.mapMode = "random"; d.start(); d.autoplay(); d.record();
  });
  ...
  writeFileSync("artifacts/bot-movement-browser.json", JSON.stringify({
    date: new Date().toISOString(), chrome: browser.version(), checks, report, errors,
  }, null, 2));
  console.log(JSON.stringify({ fps: report.fps, simulationMean: report.simulationMean,
    frameP99: report.frameP99, errors }));
