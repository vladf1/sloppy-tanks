...
try {
  ...
  await page.evaluate(() => {
    ...
    s.tanks = [t]; s.shots = []; s.events = []; s.pickups = s.pickups.filter(p => p.kind === "laser");
    t.body.setTranslation({ x: 0, y: 0.65, z: 5 }, true); t.previous = { x: 0, z: 5 };
    t.protection = 0; d.view.reset(s); d.view.zoom = 23; s.world.step();
  ...
  await page.keyboard.down("w"); await advance(25); await page.keyboard.up("w"); await advance(5);
  assert.ok(await page.evaluate(() => window.sloppy.sim.human.laser > 5));
  assert.match(await page.locator("#toast").innerText(), /50% INTERCEPT CHANCE/);
  ...
  assert.equal(await page.evaluate(() => window.sloppy.sim.pickups[0].available), false);
  checks.push("Rare pickup starts hidden, appears after 25 seconds, real W-key collection activates defense and HUD");
  // Pin the fixture for a visible successful rocket interception; the real game loop handles it.
  await page.mouse.click(800, 350); await page.evaluate(() => window.sloppy.audio.start());
  await page.waitForFunction(() => window.sloppy.audio.sounds.laser.state() === "loaded");
  await page.evaluate(() => {
    const d = window.sloppy, s = d.sim, t = s.human;
    s.shots = []; s.events = [];
    t.body.setLinvel({ x: 0, y: 0, z: 0 }, true); t.body.setTranslation({ x: 0, y: 0.65, z: 0 }, true);
    t.previous = { x: 0, z: 0 }; t.laser = 6; s.world.step(); d.view.reset(s);
    window.laserCalls = 0;
    const sound = d.audio.sounds.laser, play = sound.play.bind(sound);
    sound.play = (...args) => { if (!args.length) window.laserCalls++; return play(...args); };
    window.originalRandom = s.rng.next.bind(s.rng); s.rng.next = () => 0;
    s.shots.push({ id: s.nextId++, x: -4, z: -4, vx: 12, vz: 12,
      team: 1 - t.team, owner: -99, weapon: "rocket", damage: 65, bounces: 0, piercing: 0, life: 3.5 });
    window.advanceFrame(17);
  });
  ...
  await page.keyboard.press("Escape"); await advance(60);
  assert.equal(await page.evaluate(() => window.sloppy.sim.human.laser), beforePause);
  await page.locator("#resume").click(); await advance(365);
  ...
  checks.push("Rocket vaporizes with a thin beam and saved zap, no damage; beam expires, pause freezes timer, six-second expiry clears lens and HUD");
  ...
  await live.evaluate(() => {
    ...
    d.start(); d.autoplay(); window.intercepts = 0;
    const event = d.view.event.bind(d.view); d.view.event = (...args) => {
      if (args[0].type === "laser") window.intercepts++; event(...args);
    };
    const step = d.sim.step.bind(d.sim);
    d.sim.step = (...args) => {
      // Deliberately stronger than the rare pickup's normal single-tank workload.
      for (const t of d.sim.tanks) { t.laser = 6; t.rapid = 12; t.ammo = { spread: 36, rocket: 24, ricochet: 48, piercing: 48 }; }
      step(...args);
    };
    d.record();
  });
  await live.waitForTimeout(20000);
  const report = await live.evaluate(() => ({ ...window.sloppy.stop(), interceptions: window.intercepts }));
  assert.ok(report.interceptions > 0); assert.ok(report.snapshot.elapsed > 18); assert.deepEqual(errors, []);
  writeFileSync("artifacts/laser-defense-results.json", JSON.stringify({
  ...
  console.log(JSON.stringify({ checks, resources, fps: report.fps, p99: report.frameP99,
    simulationMean: report.simulationMean, renderMean: report.renderMean, interceptions: report.interceptions, errors }, null, 2));
