...
try {
  ...
  const target = await page.evaluate(async () => {
    ...
    const sound = d.audio.sounds.promotion,
      play = sound.play.bind(sound);
    sound.play = (...args) => {
      if (!args.length) window.promotionsHeard++;
      return play(...args);
    };
    ...
    await page.evaluate(
      () =>
        window.sloppy.view.bars
          .get(window.sloppy.sim.human.id)
          .userData.ranks.filter((c) => c.visible).length,
    ),
    1,
  );
  await page.waitForTimeout(180);
  await page.screenshot({ path: `${out}/player-promotion.png` });
  checks.push(
    "Real mouse aim and firing earn hull-damage XP, promote to Veteran, show rank beside tank name/chevron/toast and play the saved chime",
  );
  ...
  assert.equal(
    await page.evaluate(
      () =>
        window.sloppy.view.bars
          .get(window.sloppy.sim.tanks[1].id)
          .userData.ranks.filter((c) => c.visible).length,
    ),
    3,
  );
  await page.waitForTimeout(180);
  await page.screenshot({ path: `${out}/elite-and-heroic-bot.png` });
  ...
  checks.push(
    "Bot earns Heroic rank through damage with three small chevrons; Elite repair waits five combat-free seconds and freezes while paused",
  );
  await page.evaluate(async () => {
    const s = window.sloppy.sim;
    s.human.xp = 1499;
    s.damageTank(s.tanks[1], 1, s.human.id, s.human.team);
    window.advanceFrame(17);
  });
  await advance();
  ...
  await advance();
  await page.waitForTimeout(180);
  await page.screenshot({ path: `${out}/compact-hud.png` });
  ...
  await page.evaluate(() => {
    const s = window.sloppy.sim;
    s.damageTank(s.human, 9999, s.human.id, s.human.team);
  });
  await advance(185);
  ...
  checks.push(
    "Heroic rank sits beside the tank name, narrow HUD fits, death/respawn clears experience, bonuses and rank chevrons",
  );
  const resources = await page.evaluate(async () => {
    const d = window.sloppy,
      sound = d.audio.sounds.promotion;
  ...
  await live.evaluate(() => {
    ...
    d.start();
    d.autoplay();
    window.naturalPromotions = 0;
    const event = d.view.event.bind(d.view);
    d.view.event = (...args) => {
      if (args[0].type === "promotion") window.naturalPromotions++;
      event(...args);
    };
    d.record();
  });
  ...
  const report = await live.evaluate(() => ({
    ...window.sloppy.stop(),
    promotions: window.naturalPromotions,
  }));
  assert.ok(report.promotions > 0);
    ...
    JSON.stringify(
      {
        ...
        naturalPromotions: report.promotions,
        errors,
      },
      null,
      2,
