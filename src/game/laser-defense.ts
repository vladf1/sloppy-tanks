...
export function laserContactTime(
  ...
  if (
    ...
    tank.laser <= 0 ||
    tank.team === shot.team ||
    shot.laserCheckedBy?.includes(tank.id)
  ) {
    return null;
  }
  ...
  const x = end.x - tx * (frameDelta - elapsed);
  const z = end.z - tz * (frameDelta - elapsed);
  const dx = shot.x - x;
  const dz = shot.z - z;
  const vx = shot.vx - tx;
  const vz = shot.vz - tz;
  ...
  if (speed2 < 1e-8 || approach >= 0) {
    return null;
  }
  const distance2 = dx * dx + dz * dz;
  // Ignore shots traveling away or passing safely to the side.
  if (distance2 - (approach * approach) / speed2 > LASER_DEFENSE.threatRadius ** 2) {
    return null;
  }
  ...
  return simulation.visible(
    { x: x + tx * time, z: z + tz * time },
    { x: shot.x + shot.vx * time, z: shot.z + shot.vz * time },
  )
    ? time
    : null;
}
