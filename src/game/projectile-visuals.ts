...
const MODEL_SCALE: Record<Weapon, number> = {
  standard: 1,
  spread: 0.9,
  rocket: 0.65,
  ricochet: 0.65,
...
function rocketExhaust() {
  const geometry = painted([
    [point(0.16, 0.64, 0).rotateY(Math.PI).translate(0, 0, -0.82), 0xff671c],
  ]);
  const positions = geometry.getAttribute("position");
  const colors = geometry.getAttribute("color");
  const hot = new THREE.Color(0xfff2b0);
  const tip = new THREE.Color(0xff671c);
...
function model(weapon: Weapon): {
  body: THREE.BufferGeometry;
  team: THREE.BufferGeometry;
  exhaust?: THREE.BufferGeometry;
} {
  const accent = WEAPONS[weapon].color;
  const dark = 0x263344;
  const steel = 0xe2e9ef;
  switch (weapon) {
    case "standard":
      return {
        body: painted([
    ...
    case "spread":
      return {
        body: painted([
          [new THREE.IcosahedronGeometry(0.18, 1), dark],
    ...
    case "ricochet":
      return {
        body: painted([
          [new THREE.CylinderGeometry(0.31, 0.31, 0.16, 6), dark],
          [new THREE.CylinderGeometry(0.24, 0.31, 0.075, 6).translate(0, 0.117, 0), steel],
          [
            new THREE.TorusGeometry(0.255, 0.037, 4, 12).rotateX(Math.PI / 2).translate(0, 0.15, 0),
            accent,
          ],
        ]),
  ...
  constructor() {
    ...
    for (const weapon of AMMO_ORDER) {
      ...
      for (const part of [geometry.body, geometry.team, geometry.exhaust]) {
        part?.scale(size, size, size);
      }
      this.batches[weapon] = {
        body: layer(geometry.body, bodyMaterial),
        team: layer(geometry.team, teamMaterial),
  ...
  update(shots: readonly Shot[], time: number): void {
    ...
    for (let i = 0; i < Math.min(shots.length, CAPACITY); i++) {
      ...
      if (batch.exhaust) {
        ...
        pose.updateMatrix();
        batch.exhaust.setMatrixAt(index, pose.matrix);
        batch.exhaust.count++;
      }
    }
    for (const mesh of this.group.children as THREE.InstancedMesh[]) {
